...
R1=[];
R1 = transpose(R) - ((avg - Input(num,3)*ones(17500,1))); % Load Shifting (The shifted load value could be negative)
...
for i=1:17500  % A loop to partially remove negative values after shifting
    if(R1(i,1) <= 0)
        while(R1(i,1) <= 0)
            ...
            avg = mean(R);
            R1 = [];
            R1 = transpose(R) - ((avg - Input(num,3)*ones(17500,1))); % Load Shifting  after recalculation of average
        end
    end
...
R1val=[];
R1val = transpose(Rval) - ((avg - Input(num,3)*ones(4800,1))); % Load Shifting (The shifted load value could be negative)
...
for i=1:4800 % A loop to partially remove negative values after shifting
    if(R1val(i,1) <= 0)
        while(R1val(i,1) >= 0)
            ...
            avg = mean(Rval);
            R1val = [];
            R1val = transpose(Rval) - ((avg - Input(num,3)*ones(4800,1))); % Load Shifting  after recalculation of average
        end
    end
end % There could still be negative values of load even after this loop
...
Rftest = [];
cnt_test=1; % 'cnt_test' represents the variable for number of validation samples
m=1;

while((cnt_test<=1000) && (m <= 2100))
    if(R1test(m,1) >=0)
        Rftest(cnt_test,1) = R1test(m,1); % 'Rftest' represents the array to store training load samples
        cnt_test = cnt_test + 1;
    end
    m = m+1;
end
