...
R1=[];
R1 = transpose(R) - ((avg - Input(num,4)*ones(175000,1)));  % Load Shifting (The shifted load value could be negative)
...
for i=1:175000  % A loop to partially remove negative values after shifting
    if(R1(i,1) <= 0)
        while(R1(i,1) <= 0)
            ...
            avg = mean(R);
            R1 = [];
            R1 = transpose(R) - ((avg - Input(num,4)*ones(175000,1))); % Load Shifting after recalculation of average
        end
    end
...
R1val=[];
R1val = transpose(Rval) - ((avg - Input(num,4)*ones(48000,1))); % Load Shifting (The shifted load value could be negative)
...
for i=1:48000 % A loop to partially remove negative values after shifting
    if(R1val(i,1) <= 0)
        while(R1val(i,1) <= 0)
            ...
            avg = mean(Rval);
            R1val = [];
            R1val = transpose(Rval) - ((avg - Input(num,4)*ones(48000,1))); % Load Shifting  after recalculation of average
        end
    end
end  % There could still be negative values of load even after this loop
