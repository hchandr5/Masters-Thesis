tic
File=xlsread('ACTIVISg2000_load_time_series_MVAR.csv');   % One day real load data
...
if(mpc.bus(busno,3)~=0)
    if(mpc.bus(busno,4)==0)
        P118(:,num) = zeros(8500,1);
        Pval118(:,num) = zeros(2700,1);
...
R1=[];
R1 = transpose(R) - ((avg - Input(num,4)*ones(17500,1)));  % Load Shifting (The shifted load value could be negative)

for i=1:17500  % A loop to partially remove negative values after shifting
    if(R1(i,1) <= 0)
        while(R1(i,1) <= 0)
            R(i) = random(pd5);
            avg = mean(R);
            R1 = [];
            R1 = transpose(R) - ((avg - Input(num,4)*ones(17500,1))); % Load Shifting after recalculation of average
        end
    end
...
while(cnt_train<=8500) && (m <= 17500)
    if(R1(m,1) >=0)
        Rf(cnt_train,1) = R1(m,1);
...
R1val=[];
R1val = transpose(Rval) - ((avg - Input(num,4)*ones(4800,1))); % Load Shifting (The shifted load value could be negative)

for i=1:4800 % A loop to partially remove negative values after shifting
    if(R1val(i,1) <= 0)
        while(R1val(i,1) <= 0)
            Rval(i) = random(pd5);
            avg = mean(Rval);
            R1val = [];
            R1val = transpose(Rval) - ((avg - Input(num,4)*ones(4800,1))); % Load Shifting  after recalculation of average
        end
    end
end  % There could still be negative values of load even after this loop
...
m=1;
while(cnt_val<=2700) && (m <= 4800)
    if(R1val(m,1) >=0)
        Rfval(cnt_val,1) = R1val(m,1);
        cnt_val = cnt_val +1;
    end
    m = m+1;
end
...
R1test=[];
R1test = transpose(Rtest) - ((avg - Input(num,4)*ones(2000,1))); % Load Shifting (The shifted load value could be negative)

for i=1:2000 % A loop to partially remove negative values after shifting
    if(R1test(i,1) <= 0)
        while(R1test(i,1) <= 0)
            Rtest(i) = random(pd5);
            avg = mean(Rtest);
            R1test = [];
            R1test = transpose(Rtest) - ((avg - Input(num,4)*ones(2000,1))); % Load Shifting  after recalculation of average
        end
    end
end % There could still be negative values of load even after this loop
